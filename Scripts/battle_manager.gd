...
@onready var turn_action_buttons: BoxContainer = $CanvasLayer/TurnActionButtons
...
func _ready() -> void:
	...
	enemy_battlers = get_tree().get_first_node_in_group("EnemyBattler")
	...
	attack_button.pressed.connect(_show_target_button)
	...
	for e in enemy_battlers:
		e.be_selected.connect(_next_turn)
...
func _sort_turn_order_ascending(battler_1, battler_2) -> void:
	if battler_1.stats_resource.turn_speed < battler_2.stats_resource.turn_speed:
		return true
	return false
	
func _update_turn() -> void:
	if current_turn.stats_resouce.type == BattlerStats.BattlerType.PLAYER:
